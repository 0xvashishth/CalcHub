...
rowsElement = document.querySelector("select[name=rowsAdd]");
columnsElement = document.querySelector("select[name=columnsAdd]");
...
const handleInput = () => {
  ...
  InputVals += `<button class="calculateButton" onClick='handleAdd()'>Add</button>`;
...
const handleAdd = () => {
  ...
  for (i = 0; i < NoRows; i++) {
    ...
    for (j = 0; j < NoColumns; j++) {
      ...
      resultgrid += `<td><input class="result result${i}${j}" type="number" value="${value1+value2}" disabled></td>`;
    }
    resultgrid += `</tr>`;
  }
